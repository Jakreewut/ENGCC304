...
#define N 7 
#define DEN_CNT 8
//กำหนดเงิน ธนบัตร/เหรียญที่รองรับ จากมาก - น้อย
const int DENOM[DEN_CNT] = {1000, 500, 100, 50, 20, 10, 5 , 2, 1};

void clearInput() { int c; while ((c = getchar()) != '\n' && c != EOF) {} }

void showCashBox(const int cashQty[] ) {
    int sum = 0;
    printf("\n===============CAST BOX===============");
    for (int i = 0; i < DEN_CNT; ++i)
    {
        int line = DENOM[i] * cashQty[i];
        sum += line;
        if (DENOM[i] >= 20)
        {
            printf("Bank %4d : %3d pcs = %7d Baht\n", DENOM[i], cashQty[i], line);
        } else printf("Coin %4d : %3d pcs = %7d Baht\n", DENOM[i], cashQty[i], line); 
    } 
    printf("---------------------\ntotal cash in box = %d Baht\n", sum);    
}

//เงินทอน จะหักจริงเมื่อสำเร็จ
int giveChangeWithInventory;
...
const int DENOM[DEN_CNT] = {1000, 500, 100, 50, 20, 10, 5 , 2, 1};
        ...
        else if (menu == 8)
        {
            ...
            printf("\n Cancel /%s/ | Delete %d intems | Remaining %d | New stock %d | Tatal = %d Baht", name[idx], qty, cart[idx], stock[idx], total);
            continue;
        }
